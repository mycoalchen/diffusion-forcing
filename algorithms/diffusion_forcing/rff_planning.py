...
from .rff_base import RectifiedFlowForcingBase
...
class RectifiedFlowForcingPlanning(RectifiedFlowForcingBase):
    ...
    def training_step(self, batch, batch_idx):
        ...
        xs_pred, loss = self.rf_model(xs, conditions, noise_levels=self._generate_noise_levels(xs, masks=masks))
    ...
    def plan(self, start: torch.Tensor, goal: torch.Tensor, horizon: int, conditions: Optional[Any] = None):
        ...
        chunk = torch.clamp(chunk, -self.cfg.rf.clip_noise, self.cfg.rf.clip_noise)
        ...
        for m in range(scheduling_matrix.shape[0] - 1):
            ...
            plan[1 : self.n_tokens - pad_tokens] = self.rf_model.sample_step(
