...
from .models.rf import RectifiedFlow


class RectifiedFlowForcingBase(BasePytorchAlgo):
    def __init__(self, cfg: DictConfig):
        ...
        self.timesteps = cfg.rf.timesteps
        self.sampling_timesteps = cfg.rf.sampling_timesteps
        self.clip_noise = cfg.rf.clip_noise

        super().__init__(cfg)
        self.cfg.rf.cum_snr_decay = self.cfg.rf.cum_snr_decay ** (self.frame_stack * cfg.frame_skip)

        self.validation_step_outputs = []

    def _build_model(self):
        self.rf_model = RectifiedFlow(
    ...
    def configure_optimizers(self):
        params = tuple(self.rf_model.parameters())
    ...
    def training_step(self, batch, batch_idx) -> STEP_OUTPUT:
        ...
        xs_pred, loss = self.rf_model(xs, conditions, noise_levels=self._generate_noise_levels(xs))
    ...
    def validation_step(self, batch, batch_idx, namespace="validation") -> STEP_OUTPUT:
        ...
        while curr_frame < n_frames:
            ...
            for m in range(scheduling_matrix.shape[0] - 1):
                ...
                xs_pred[start_frame:] = self.rf_model.sample_step(
                    xs_pred[start_frame:],
    ...
    def reweight_loss(self, loss, weight=None):
        # Note there is another part of loss reweighting (fused_snr) inside the Rectified Flow class!
